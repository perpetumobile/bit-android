...
public class ConfigServerManager {
	...
	protected BatchHttpRequest batchHttpRequest = new BatchHttpRequest(Config.getInstance().getProperty(CONFIG_SERVER_MANAGER_HTTP_REQUEST_CLASS_KEY, null), CONFIG_SERVER_MANAGER_THREAD_POOL_NAME) {
		@Override
		protected void onResponse(ArrayList<HttpResponseDocument> result) {
			if(save(Config.CONFIG_PROPERTIES_VERSION_DIRECTORY_PATH(), result)) {
